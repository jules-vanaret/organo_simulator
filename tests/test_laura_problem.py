...
for index_simu in range(10):
    ...
    persistence_times = np.array([100]*(N_part))
    viscosities = np.array([1]*(N_part))
    Ds = np.array([1e-6]*(N_part)) # diffusion coefficients
    ...
    simulator = FastOverdampedSimulator(
        ...
        energy_potential=1.4e-4,
        parallel=True
    )

    skip = 500 # return coordinates every s
    ...
    np.save(
        f'{path_to_save}/tracks_{index_simu}.npy',
        tracks[75:]
    )
