...
merge_rates_str = [str(elem).zfill(3) for elem in np.arange(5,10,5)]
...
for merge_rate in merge_rates_str:
    ...
    all_points = []
    all_facecolors = []
    path2coords = f'{path2data}/coords_merge_{merge_rate}'
    path2dicts = f'{path2data}/dicts_merge_{merge_rate}'
