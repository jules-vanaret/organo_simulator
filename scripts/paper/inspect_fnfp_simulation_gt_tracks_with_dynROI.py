...
fnfp_rates_str = [str(elem).zfill(3) for elem in np.arange(45,50,5)]

for fnfp_rate in fnfp_rates_str:

    name = f'coords_fnfp_{fnfp_rate}'

    all_points = []
    all_facecolors = []
    path2coords = f'{path2data}/coords_fnfp_{fnfp_rate}'
    path2dicts = f'{path2data}/dicts_fnfp_{fnfp_rate}'
